...
{
	const Color color{ 0xAA, 0xBB, 0xCC };
...
TEST(readScanLines, Color)
{
	...
	const Color actual{ scanLines[1][0] };

	CHECK_EQUAL(0x33, actual.getRed());
	CHECK_EQUAL(0x22, actual.getGreen());
	CHECK_EQUAL(0x11, actual.getBlue());
}