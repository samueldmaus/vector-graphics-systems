...
namespace Framework
{
	Xml::HElement SceneWriter::writeScene(Scene& scene)
	{
		...
		for (size_t i = 0; i < scene.getLayerCount(); ++i)
		{
			...
			//	pGraphicElement.addAttribute("x", std::to_string(scene.getLayer(i).getGraphic(i).getPlacementPoint().getX()));
			//	pGraphicElement.addAttribute("y", std::to_string(scene.getLayer(i).getGraphic(i).getPlacementPoint().getY()));
